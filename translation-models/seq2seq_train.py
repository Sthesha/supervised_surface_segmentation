...
class Seq2SeqTrainer:
    ...
    def get_or_build_tokenizer(self, config, lang):
        ...
        if tokenizer_file_path.exists():
            ...
            if self.config['tokenize_custom'][lang]:
                tokenizer = SyllableTokenizer.from_file(str(tokenizer_file_path), mode=tokenize_method)
                tokenizer.finalize_vocab()  # Finalize vocabulary after loading
                print("Vocabulary finalized for inference")
            else:
            ...
            train_sentences = [item['translation'][lang] for item in self.train_ds_raw 
                 if isinstance(item['translation'][lang], str)]
            ...
            if self.config['tokenize_custom'][lang]:
                ...
                tokenizer.encode_batch(train_sentences, add_special_tokens=False)
                print("Pruning vocabulary...")
                tokenizer.prune_vocab()
                print(f"Vocabulary size after pruning: {tokenizer.get_vocab_size()}")
                
                # Finalize vocabulary before saving
                tokenizer.finalize_vocab()
                print("Vocabulary finalized")
    ...
    def setup_datasets(self):
        ...
        self.train_ds_raw, self.val_ds_raw, self.test_ds_raw = random_split(
        ...
        train_ds = BilingualDataset(self.train_ds_raw,
                                   self.tokenizer_src,
                                   self.tokenizer_tgt,
        ...
        val_ds = BilingualDataset(self.val_ds_raw,
                                 self.tokenizer_src,
                                 self.tokenizer_tgt,
                                 self.config['lang_src'],
                                 self.config['lang_tgt'],
        ...
        test_ds = BilingualDataset(self.test_ds_raw,
                                  self.tokenizer_src,
                                  self.tokenizer_tgt,
                                  self.config['lang_src'],
                                  self.config['lang_tgt'],
        ...
        max_len_src = max([len(self.tokenizer_src.encode(item['translation'][self.config['lang_src']]).ids) for item in ds_raw])
        max_len_tgt = max([len(self.tokenizer_tgt.encode(item['translation'][self.config['lang_tgt']]).ids) for item in ds_raw])
    ...
    def validate(self, epoch, writer):
        ...
        references = [" ".join(t.split()) for t in expected]
        hypotheses = [" ".join(p.split()) for p in predicted]
    ...
    def generate_predictions(self):
        ...
        with torch.no_grad():
            for batch in tqdm(self.test_dataloader, desc=f"Evaluating {len(self.test_dataloader)} Samples"):
                ...
                src_text = batch["src_text"][0].replace("-","")
                tgt_text = batch["tgt_text"][0].replace("-","")
                model_out_text = self.tokenizer_tgt.decode(model_out.detach().cpu().numpy(), skip_special_tokens=True).replace("-","")
    ...
    def get_predictions_from_file(self, file_path=None):
        ...
        if file_path is None:
            pred_dir = os.path.join(self.config['file_path'], 'external_predictions')
    ...
    def create_dataloader_from_df(self, df):
        """
        Create a DataLoader from a DataFrame containing source and target text pairs
        
        Args:
            df (pd.DataFrame): DataFrame containing source and target language columns
            
        Returns:
            DataLoader: DataLoader containing the processed data
        """
        # Convert DataFrame to the format expected by BilingualDataset
        data_raw = [
            {
                "translation": {
                    self.config['lang_src']: row[self.config['lang_src']],
    ...
    def evaluate_on_test(self, external_test_data):
        """First create a DataLoader from the external test data, then evaluate"""
        # Create DataLoader from the external test data
        test_dataloader = self.create_dataloader_from_df(external_test_data)
        ...
        with torch.no_grad():
            for batch in tqdm(test_dataloader, desc="Evaluating on External Test Data"):
                try:
                    # Get the source text and target text
                    src_text = batch["src_text"][0]  # batch size is 1
                    ...
                    # Translate using the existing method
                    model_out = self.greedy_decode(
                        self.model,
                        encoder_input,
                        encoder_mask,
                    ...
                    # Show examples at regular intervals
                    if len(test_pred_texts) % (len(test_dataloader) // 4) == 0:
                        print(f"\nExample translation:")
        ...
        # Save results to CSV
        if results:
            save_dir = os.path.join(self.config['file_path'], 'test_predictions')
            os.makedirs(save_dir, exist_ok=True)
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            csv_path = os.path.join(save_dir, f'test_predictions_{timestamp}.csv')
            
            df = pd.DataFrame(results)
            df.to_csv(csv_path, index=False, encoding='utf-8')
            print(f"\nPredictions saved to: {csv_path}")
        
        return test_tgt_texts, test_pred_texts
    ...
    def save_evaluation_results(self, model_path, bleu_scores, chrf_score, sacre_bleu):
        ...
        txt_path = os.path.join(model_path, 'evaluation_results.txt')
        ...
        with open(txt_path, 'w') as f:
            ...
            f.write(f"Simple scores: {results['BLEU Scores']['Simple scores']:.4f}\n\n")
             ...
             # SACRE BLEU scores
            f.write(f"Sacre BLEU: {results['sacreBLEU Scores']['SACRE BLEU']:.4f}\n")
            f.write(f"Sacre chrF: {results['sacreBLEU Scores']['SACRE chrF']:.4f}\n\n")
    ...
    def evaluate_external_data(self, external_data_df):
        """
        Evaluate the model on external data and save predictions
        
        Args:
            external_data_df (pd.DataFrame): DataFrame containing external test data
            
        Returns:
            tuple: (bleu_score, chrf_score, predictions_file_path)
        """
        # Convert DataFrame to list of translation dictionaries
        ...
        with torch.no_grad():
            for item in tqdm(test_data, desc="Evaluating External Data"):
                ...
                encoder_input = torch.cat([
                    ...
                    torch.tensor([self.tokenizer_src.token_to_id('[EOS]')], dtype=torch.int64),
                    torch.tensor([self.tokenizer_src.token_to_id('[PAD]')] * 
                               (self.config['seq_len'] - len(encoder_input.ids) - 2), 
                               dtype=torch.int64)
                ], dim=0).to(self.device)
    
                encoder_mask = (encoder_input != self.tokenizer_src.token_to_id('[PAD]')) \
                              .unsqueeze(0).unsqueeze(0).int().to(self.device)
                ...
                if len(all_results) % (len(test_data) // 4) == 0:
                    print(f"\nSample prediction {len(all_results)}:")
                    print(f"SOURCE: {src_text}")
                    print(f"TARGET: {tgt_text}")
                    print(f"PREDICTED: {model_out_text}")
    
        # Save predictions
        save_dir = os.path.join(self.config['file_path'], 'external_predictions')
        os.makedirs(save_dir, exist_ok=True)
        
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        csv_path = os.path.join(save_dir, f'external_predictions_{timestamp}.csv')
        ...
        # Calculate metrics
        predictions = [result['predicted'] for result in all_results]
        ...
        bleu_scores = self.eval_bleu_segment(predictions, targets)
        chrf_score = self.eval_chrF_segment(predictions, targets)
        sacre_bleu = self.eval_sacrebleu_segment(predictions, targets)
    
        # Save evaluation results
        self.save_evaluation_results(save_dir, bleu_scores, chrf_score, sacre_bleu)
    
        # Log metrics to tensorboard
        if hasattr(self, 'writer') and self.writer is not None:
            self.writer.add_scalar('external_test/bleu_score', bleu_scores['simple'])
            self.writer.add_scalar('external_test/chrf_score', chrf_score)
            self.writer.add_scalar('external_test/sacrebleu_score', sacre_bleu['sacre_bleu'])
            self.writer.flush()
